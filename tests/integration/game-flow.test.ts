...
test("Complete game tick integration - all systems working together", () => {
    ...
    expect(state.entities["npc-1"]?.components.health).toEqual({ current: 50, max: 50 });
    expect(state.entities["npc-1"]?.components.position).toEqual({ x: 10, y: 10 });

    // 2. Player moved AND took damage (both systems affected same entity)
    expect(state.entities["player-1"]?.components.position).toEqual({ x: 20, y: 20 }); // Moved EAST
    expect(state.entities["player-1"]?.components.health).toEqual({ current: 70, max: 100 }); // 70 - 30 damage

    // 3. Enemy moved AND healed (both systems affected same entity)
    expect(state.entities["enemy-1"]?.components.position).toEqual({ x: 17, y: 1 }); // Moved NORTH (y: 1 + 1 -> 2)
    expect(state.entities["enemy-1"]?.components.health).toEqual({ current: 30, max: 50 }); // 30 + 10 health
});